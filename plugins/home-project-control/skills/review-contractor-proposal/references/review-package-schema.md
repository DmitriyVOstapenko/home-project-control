...
`ready_for_owner` требует полной инвентаризации и чтения точной версии, полного покрытия требований и строк КП, отсутствия существенных блокеров, отдельного сопоставимого подрядчика или поставщика в зарегистрированном внешнем поиске и совпадающего `completion_manifest`. `ready_for_contract` дополнительно требует условно положительного вердикта, проверяемого диапазона стоимости, закрытых проверок на объекте и завершённой оценки подрядчика. Ни одно из этих состояний не является решением владельца.
